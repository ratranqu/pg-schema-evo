...
Version 0.6.0 — Data masking library (`DataMasking`): a standalone, pure-Swift module for data anonymization and obfuscation. 8 built-in strategies (hash, fake, redact, partial, null, preserve-format, regex, numeric-noise) with extensible registry for custom strategies. Value parsers (email, phone, IP, credit card, SSN) using swift-parsing for structured decomposition. DSL expression language for complex per-field transforms (e.g. `hash(email.local) + "@" + email.domain`). YAML and programmatic configuration. `mask` CLI command. Benchmark suite (ordo-one/package-benchmark) with zero-allocation passthrough verification. Comprehensive test coverage at 90%+ with 1019+ tests across 84+ suites.
