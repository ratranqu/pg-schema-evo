...
Version 0.4.4 — Extended integration tests batch 2: 14 new tests covering constraint/index diff detection with ALTER generation, column type and default change detection, view-on-view cascade dependency ordering, env var interpolation in YAML config (with defaults and undefined var errors), PL/pgSQL function introspection with DECLARE/EXCEPTION blocks, syncAll schema mode, enum label ordering preservation, wide table introspection (50 columns), composite type cloning, sequence parameter preservation (INCREMENT/MINVALUE/MAXVALUE/CACHE/CYCLE), and mixed multi-object-type cloning with type ordering. Prior: 12 integration tests for config loading, schema diff, sync, clone error paths. Improved code coverage to 85%+, performance enhancements (connection pooling, parallel data transfer), schema migration with diff --sql, safety flags, incremental data sync, YAML config files, partitioned tables, RLS policies, selective data filters, pre-flight validation, and retry with rollback.
