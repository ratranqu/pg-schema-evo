...
struct SchemaDifferTests {
    ...
    // MARK: - Identity column comparison (Item 3)

    @Test("Identity added to column generates ADD GENERATED AS IDENTITY")
    func identityAdded() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1, isIdentity: true, identityGeneration: "ALWAYS"),
        ])
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1, isIdentity: false),
        ])
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("identity added") && $0.contains("ALWAYS") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("ADD GENERATED ALWAYS AS IDENTITY") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("DROP IDENTITY") })
    }

    @Test("Identity removed from column generates DROP IDENTITY")
    func identityRemoved() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1, isIdentity: false),
        ...
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1, isIdentity: true, identityGeneration: "BY DEFAULT"),
        ])
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("identity removed") && $0.contains("BY DEFAULT") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("DROP IDENTITY") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("ADD GENERATED BY DEFAULT AS IDENTITY") })
    }

    @Test("Identity generation changed from ALWAYS to BY DEFAULT")
    func identityGenerationChanged() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1, isIdentity: true, identityGeneration: "BY DEFAULT"),
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("identity generation") && $0.contains("ALWAYS -> BY DEFAULT") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("SET GENERATED BY DEFAULT") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("SET GENERATED ALWAYS") })
    }

    @Test("Identical identity columns produce no diff")
    func identityIdentical() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1, isIdentity: true, identityGeneration: "ALWAYS"),
    ...
    // MARK: - Precision/length comparison (Item 4)

    @Test("Character max length difference detected")
    func characterMaxLengthDifference() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "name", dataType: "character varying", isNullable: false, ordinalPosition: 1, characterMaximumLength: 255),
        ])
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "name", dataType: "character varying", isNullable: false, ordinalPosition: 1, characterMaximumLength: 100),
        ])
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("character max length") && $0.contains("100 -> 255") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("TYPE character varying(255)") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("TYPE character varying(100)") })
    }

    @Test("Character max length removed detected")
    func characterMaxLengthRemoved() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "name", dataType: "character varying", isNullable: false, ordinalPosition: 1),
        ])
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "name", dataType: "character varying", isNullable: false, ordinalPosition: 1, characterMaximumLength: 100),
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("character max length") && $0.contains("100 -> unlimited") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("TYPE character varying") && !$0.contains("(") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("TYPE character varying(100)") })
    }

    @Test("Numeric precision/scale difference detected")
    func numericPrecisionDifference() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "products")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "price", dataType: "numeric", isNullable: false, ordinalPosition: 1, numericPrecision: 12, numericScale: 4),
        ])
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "price", dataType: "numeric", isNullable: false, ordinalPosition: 1, numericPrecision: 10, numericScale: 2),
        ])
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("numeric precision/scale") && $0.contains("(10,2) -> (12,4)") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("TYPE numeric(12,4)") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("TYPE numeric(10,2)") })
    }

    @Test("Numeric precision added where none existed")
    func numericPrecisionAdded() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "products")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "amount", dataType: "numeric", isNullable: false, ordinalPosition: 1, numericPrecision: 8, numericScale: 2),
        ])
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "amount", dataType: "numeric", isNullable: false, ordinalPosition: 1),
        ])
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("numeric precision/scale") })
        #expect(result.modified[0].migrationSQL.contains { $0.contains("TYPE numeric(8,2)") })
        #expect(result.modified[0].reverseMigrationSQL.contains { $0.contains("TYPE numeric") && !$0.contains("(") })
    }

    @Test("Same precision produces no diff")
    func samePrecisionNoDiff() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "products")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(id: tableId, columns: [
            ColumnInfo(name: "price", dataType: "numeric", isNullable: false, ordinalPosition: 1, numericPrecision: 10, numericScale: 2),
    ...
    // MARK: - Constraint/index definition comparison (Item 5)

    @Test("Constraint definition differs generates DROP+ADD CONSTRAINT")
    func constraintDefinitionDiffers() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "orders")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(
            id: tableId,
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            constraints: [
                ConstraintInfo(name: "orders_amount_check", type: .check, definition: "CHECK (amount > 0 AND amount < 10000)"),
            ]
        )
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("orders_amount_check") && $0.contains("definition differs") })
        let sql = result.modified[0].migrationSQL.joined(separator: "\n")
        #expect(sql.contains("DROP CONSTRAINT"))
        #expect(sql.contains("ADD CONSTRAINT"))
        #expect(sql.contains("amount < 10000"))
    }

    @Test("Constraint with same definition produces no diff")
    func constraintSameDefinition() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "orders")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(
            id: tableId,
    ...
    @Test("Index definition differs generates DROP+CREATE INDEX")
    func indexDefinitionDiffers() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(
            id: tableId,
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            indexes: [
                IndexInfo(name: "idx_users_email", definition: "CREATE INDEX idx_users_email ON public.users USING btree (email, name)", isUnique: false, isPrimary: false),
            ]
        )
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(
        ...
        #expect(result.modified.count == 1)
        #expect(result.modified[0].differences.contains { $0.contains("idx_users_email") && $0.contains("definition differs") })
        let sql = result.modified[0].migrationSQL.joined(separator: "\n")
        #expect(sql.contains("DROP INDEX"))
        #expect(sql.contains("email, name"))
    }

    @Test("Index with same definition produces no diff")
    func indexSameDefinition() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        ...
        source.tables[tableId] = TableMetadata(
            id: tableId,
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            indexes: [IndexInfo(name: "idx_users_email", definition: indexDef, isUnique: false, isPrimary: false)]
        )
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(
            id: tableId,
    ...
    @Test("Constraint definition change generates reverse SQL")
    func constraintDefinitionChangeReverse() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "orders")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(
            id: tableId,
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            constraints: [
                ConstraintInfo(name: "orders_fk", type: .foreignKey, definition: "FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE", referencedTable: "users"),
            ]
        )
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(
            ...
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            constraints: [
                ConstraintInfo(name: "orders_fk", type: .foreignKey, definition: "FOREIGN KEY (user_id) REFERENCES users(id)", referencedTable: "users"),
            ]
        )
        ...
        #expect(result.modified.count == 1)
        let reverseSql = result.modified[0].reverseMigrationSQL.joined(separator: "\n")
        #expect(reverseSql.contains("DROP CONSTRAINT"))
        #expect(reverseSql.contains("ADD CONSTRAINT"))
        #expect(reverseSql.contains("REFERENCES users(id)"))
        #expect(!reverseSql.contains("CASCADE"))
    }

    @Test("Index definition change generates reverse SQL")
    func indexDefinitionChangeReverse() async throws {
        let tableId = ObjectIdentifier(type: .table, schema: "public", name: "users")
        let source = ConfigurableDiffIntrospector(objects: [tableId])
        source.tables[tableId] = TableMetadata(
            id: tableId,
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            indexes: [
                IndexInfo(name: "idx_search", definition: "CREATE INDEX idx_search ON public.users USING gin (search_vector)", isUnique: false, isPrimary: false),
            ]
        )
        let target = ConfigurableDiffIntrospector(objects: [tableId])
        target.tables[tableId] = TableMetadata(
            ...
            columns: [ColumnInfo(name: "id", dataType: "integer", isNullable: false, ordinalPosition: 1)],
            indexes: [
                IndexInfo(name: "idx_search", definition: "CREATE INDEX idx_search ON public.users USING btree (name)", isUnique: false, isPrimary: false),
            ]
        )
        ...
        #expect(result.modified.count == 1)
        let reverseSql = result.modified[0].reverseMigrationSQL.joined(separator: "\n")
        #expect(reverseSql.contains("DROP INDEX"))
        #expect(reverseSql.contains("btree (name)"))
    }
}
